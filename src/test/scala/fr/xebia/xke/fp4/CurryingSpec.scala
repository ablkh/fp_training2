...
class CurryingSpec extends FunSpec with Matchers {
  ...
  describe("an add function with 2 arguments for integers") {
    ...
    def add(x: Int, y: Int): Int = x + y //TODO EXO2
    ...
    it("it can be manually partially applied on x", EXO_4_2) {
      val add_one: Int => Int = _ + 1 //TODO EXO2
  ...
  describe("an add function as higher order function") {
    ...
    def add(x: Int): Int => Int = _ + x //TODO EXO2
    ...
    it("it can be manually partially applied on x", EXO_4_2) {
      val add_one: Int => Int = add(1) //TODO EXO2
  ...
  def curry[A, B, C](f: ((A, B) => C)): A => B => C = f.curried //TODO EXO2

  def uncurry[A, B, C](f: (A => B => C)): (A, B) => C = (x:A, y:B) => f.apply(x).apply(y) //TODO EXO2

  describe("the curry function") {

    it("it transform any function (A,B) => C into A => B => C", EXO_4_2) {
      def add(x: Int, y: Int): Int = x + y //TODO EXO2
      val yet_another_curried_add_one: (Int) => Int = curry(add)(1) //TODO EXO2
  ...
  describe("the uncurry function") {
    ...
    it("it transform any function A => B => C into (A,B) => C", EXO_4_2) {
      def add(x: Int)(y: Int): Int = x + y //TODO EXO2
      val yet_another_uncurried_add: (Int, Int) => Int = uncurry(add) //TODO EXO2
  ...
  describe("a curried add function") {
    ...
    def add(x: Int)(y: Int): Int = x + y //TODO EXO3
    ...
    it("it can be manually partially applied on x", EXO_4_3) {
      val add_one: Int => Int = add(1) //TODO EXO3
    ...
    it("it can be uncurried", EXO_4_3) {
      val add_uncurried: (Int, Int) => Int = uncurry(add) //TODO EXO3
    ...
    it("it can be uncurried and curried", EXO_4_3) {
      val add_uncurried: (Int, Int) => Int = uncurry(add) //TODO EXO3
      val add_re_curried: Int => Int => Int = curry(add_uncurried) //TODO EXO3
